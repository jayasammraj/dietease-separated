/**
 * Excel Reporter — PancreaScan Format
...
async function generateReport(allResults, reportDir) {
  ...
  wb.creator  = 'DietEase+ Selenium Suite';
  wb.created  = new Date();
  ...
  // Header row — dark blue bg, bold white text
  ...
  // Title
  const titleRow = ws2.addRow(['DietEase+ — E2E Selenium Test Report']);
  ...
  const stats = [
    ['Generated',       genDate,    '', ''],
    ['Website',         'https://dietease-plus.surge.sh', '', ''],
  ...
  const file  = path.join(reportDir, `E2E_Test_Report_DietEasePlus_${stamp}.xlsx`);
